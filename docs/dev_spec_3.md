...
|---------|------|-------------|
| 1.4 | 2026-03-16 | Harmonized with dev_spec_1.md and dev_spec_2.md; added Unified Architecture Overview; clarified Change entity sourcing from dev_spec_2 WF2; unified role model and user auth model; established single backend for all three user stories |
...
This specification represents **User Story 3 of 3** in a harmonized backend system:
...
- **Unified Project/Board System:** Projects and boards created via WF1 (dev_spec_1.md); changes operate on these boards
- **Unified Change Entity:** Changes generated by WF2 (dev_spec_2.md) AIEngine are reviewed by WF3 (this spec) before application to boards created in WF1
...
```
WF1: ProjectManager creates Project in dev_spec_1.md
     → AIEngine generates Board + Cards
     
WF2: MeetingFacilitator creates Meeting in dev_spec_2.md
     → AIEngine extracts ActionItems, Decisions, Changes
     → Creates Change records in changes table
     → ApprovalService manages summary approval
     
WF3: TeamMember reviews Changes (created by WF2, THIS SPEC)
     → PreviewChange displays changes with diff/impact analysis
     → ApprovalService manages change approval (separate from WF2)
     → ChangeApplicationService applies approved changes to Board (created in WF1)
     → Audit trail tracks all approvals and applications
```
...
| **AP4.3.1** | `NotificationService` | Service | Optional notification service (not required) |
...
| **DM1.5** | `ApprovalRule` | Rule for approval evaluation | `ruleId: Universally Unique Identifier`, `name: String`, `conditions: List<Condition>`, `requiredApprovers: Integer` |
...
| Lost all runtime state | RF2 | All services | Session data lost, in-progress operations abandoned | Transaction recovery from database, user re-authentication, journal-based recovery |
...
|-------|-----------|---------|------------|---------|----------------|
| TECH-LANG-001 | Java 21 | Primary language | Mature, strong typing, performance | 21 Long Term Support | https://openjdk.org/projects/jdk/21/ |
| TECH-FW-001 | Spring Boot 3.x | Framework | Rapid development, ecosystem | 3.2.x | https://spring.io/projects/spring-boot |
| TECH-DB-002 | Spring Data JPA | Object-Relational Mapping/Persistence | Reduced boilerplate, pagination | 3.2.x | https://spring.io/projects/spring-data-jpa |
| TECH-DB-001 | PostgreSQL 16 | Database | ACID, reliability, JSON support | 16.1 | https://www.postgresql.org/docs/16/ |
| TECH-DB-003 | Flyway | Database versioning | Schema versioning, consistency | 10.x | https://flywaydb.org/documentation |

## Frontend Technologies

| Label | Technology | Purpose | Why Picked | Version | Documentation |
|-------|-----------|---------|------------|---------|----------------|
| TECH-FE-001 | React 18 | User Interface Framework | Components, ecosystem | 18.3.1 | https://react.dev |
| TECH-FE-002 | TypeScript 5.x | Type safety | Compile-time error detection | 5.3.x | https://www.typescriptlang.org/docs/ |
| TECH-FE-003 | Vite | Build tool | Fast development server | 6.3.5 | https://vitejs.dev/ |
| TECH-FE-004 | Tailwind CSS | Utility-first styling | Rapid development | 4.1.12 | https://tailwindcss.com/ |
| TECH-FE-005 | Radix UI | Headless components | Unstyled primitives | Latest | https://www.radix-ui.com/ |
| TECH-FE-006 | shadcn/ui | High-quality components | Built on Radix + Tailwind | Latest | https://ui.shadcn.com/ |
| TECH-FE-007 | React Router | Routing | Client-side navigation | 7.13.0 | https://reactrouter.com/ |
| TECH-FE-008 | React Hook Form | Form management | Performant, minimal re-renders | 7.55.0 | https://react-hook-form.com/ |
| TECH-FE-009 | Zustand | State management | Lightweight, predictable | 5.0.11 | https://github.com/pmndrs/zustand |
| TECH-FE-010 | Axios | HTTP client | Interceptors, error handling | 1.6.x | https://axios-http.com/docs/intro |
| TECH-FE-011 | React DnD | Drag-and-drop | Flexible, accessible | 16.0.1 | https://react-dnd.github.io/react-dnd/ |
| TECH-FE-012 | Lucide React | Icons | Consistent, accessible | 0.487.0 | https://lucide.dev/ |
| TECH-FE-013 | Sonner | Toast notifications | Beautiful, headless | Latest | https://sonner.emilkowalski.com/ |
| TECH-FE-014 | date-fns | Date manipulation | Modular, lightweight | 3.6.0 | https://date-fns.org/ |

## Infrastructure Technologies

| Label | Technology | Purpose | Why Picked | Version | Documentation |
|-------|-----------|---------|------------|---------|----------------|
| TECH-CONT-001 | Docker | Containerization | Consistent deployments | 24.0.7 | https://docs.docker.com |
| TECH-INFRA-001 | Kubernetes | Orchestration | Scaling, self-healing | 1.28.x | https://kubernetes.io/docs/ |
| TECH-INFRA-002 | Prometheus | Metrics | Monitoring, alerting | 2.48.x | https://prometheus.io/docs/ |
| TECH-INFRA-003 | ELK Stack | Logging | Analysis, debugging | 8.x | https://www.elastic.co/guide/index.html |

## Testing Technologies

| Label | Technology | Purpose | Why Picked | Version | Documentation |
|-------|-----------|---------|------------|---------|----------------|
| TECH-TEST-001 | JUnit 5 | Unit testing | Standard Java testing | 5.10.x | https://junit.org/junit5/docs/current/user-guide/ |
| TECH-TEST-003 | Mockito | Mocking | Clean Application Programming Interface, verification | 5.x | https://javadoc.io/doc/org.mockito/mockito-core/latest/org/mockito/Mockito.html |
| TECH-TEST-002 | TestContainers | Integration testing | Real database testing | 1.19.x | https://www.testcontainers.org/ |
| TECH-TEST-004 | Jest | JavaScript testing | Fast, snapshots | 29.x | https://jestjs.io/docs/getting-started |
| TECH-TEST-005 | React Testing Library | Component testing | User-centric | 14.x | https://testing-library.com/docs/react-testing-library/intro/ |
| TECH-TEST-006 | Cypress | End-to-End testing | Reliable, debugging | 13.x | https://docs.cypress.io |
...
All three development specifications (dev_spec_1, dev_spec_2, dev_spec_3) are served by a **single Spring Boot backend** running on a single instance. This specification (WF3) provides the change review and approval functionality that integrates with:

- **dev_spec_1**: Operates on projects, boards, and cards created by WF1
- **dev_spec_2**: Consumes `changes` records created by WF2's AI analysis
...
WF2 Endpoints (from dev_spec_2.md):
...
| boards | WF1 | WF1, WF3 | Kanban boards |
| cards | WF1 | WF1, WF3 | Work items/tasks |
| changes | WF2 | WF3 | Changes from meeting AI analysis (WF2 creates, WF3 reviews) |
| preview_changes | WF3 | WF3 | Preview state for changes under review |
| approval_decisions | WF3 | WF3 | Approval decisions for individual  changes |
| change_snapshots | WF3 | WF3 | Snapshots for rollback |
| audit_trail | WF3 | WF3 | Audit logging |

### Cross-Workflow Dependencies

**WF3 depends on:**
- changes table (created by WF2)
- users table (for approval authority)
- boards, cards, stages tables (from WF1 to show before/after states and apply changes)

**WF3 modifies:**
- cards table (when applying approved changes back to board)
- Creates audit trail records for all approvals and applications
...
|-------|--------|--------|----------|-------------|------------|-------------|
| AP1.1.1 | `getPreviewChanges()` | + public | `GET /api/v1/changes` | `ResponseEntity<List<PreviewChangeDataTransferObject>>` | `meetingId`, `pageable` | Get all pending changes from meeting |
| AP1.1.2 | `getChangeDetail()` | + public | `GET /api/v1/changes/{changeId}` | `ResponseEntity<PreviewDetailDataTransferObject>` | `changeId` | Get full change detail with diff |
| AP1.1.3 | `getDiff()` | + public | `GET /api/v1/changes/{changeId}/diff` | `ResponseEntity<DiffDataTransferObject>` | `changeId` | Get detailed diff visualization |
| AP1.1.4 | `getImpactAnalysis()` | + public | `GET /api/v1/changes/{changeId}/impact` | `ResponseEntity<ImpactDataTransferObject>` | `changeId` | Get impact analysis |
| AP1.1.5 | `getPreviewStats()` | + public | `GET /api/v1/changes/statistics` | `ResponseEntity<PreviewStatsDataTransferObject>` | `meetingId` | Get preview statistics |

### 11.2 ApprovalController APIs

| Label | Method | Access | Endpoint | Return Type | Parameters | Description |
|-------|--------|--------|----------|-------------|------------|-------------|
| AP2.1.1 | `approveChange()` | + public | `POST /api/v1/changes/{changeId}/approve` | `ResponseEntity<ApprovalResultDataTransferObject>` | `ApprovalRequestDataTransferObject` | Approve single change |
| AP2.1.2 | `rejectChange()` | + public | `POST /api/v1/changes/{changeId}/reject` | `ResponseEntity<RejectionResultDataTransferObject>` | `RejectionRequestDataTransferObject` | Reject single change |
| AP2.1.3 | `approveBatch()` | + public | `POST /api/v1/changes/approve-batch` | `ResponseEntity<BatchResultDataTransferObject>` | `BatchApprovalRequestDataTransferObject` | Approve multiple changes |
| AP2.1.4 | `getApprovalStatus()` | + public | `GET /api/v1/changes/{changeId}/approval-status` | `ResponseEntity<ApprovalStatusDataTransferObject>` | `changeId` | Get approval status |

### 11.3 ApplicationController APIs

| Label | Method | Access | Endpoint | Return Type | Parameters | Description |
|-------|--------|--------|----------|-------------|------------|-------------|
| AP3.1.1 | `applyChanges()` | + public | `POST /api/v1/changes/{changeId}/apply` | `ResponseEntity<ApplicationResultDataTransferObject>` | `ApplicationRequestDataTransferObject` | Apply approved changes |
| AP3.1.2 | `verifyApplication()` | + public | `GET /api/v1/changes/{changeId}/verify` | `ResponseEntity<VerificationDataTransferObject>` | `changeId` | Verify application success |
| AP3.1.3 | `rollbackChange()` | + public | `POST /api/v1/changes/{changeId}/rollback` | `ResponseEntity<RollbackResultDataTransferObject>` | `changeId` | Rollback applied change |
...
|----------|-----------|-----------------|-------------|----------------|
| **Java** | Spring Boot | RestTemplate / Webclient | `/api/v1/changes` | See ApprovalService javadoc |
| **C#** | .NET 6+ | HttpClient | `GET /api/v1/changes` | See GitHub: CSharp-ApprovalClient |
| **Python** | Flask/Django | requests library | `requests.get('/api/v1/changes')` | See pip: approval-client-python |
| **JavaScript/TypeScript** | React / Node.js | Axios / Fetch API | `await api.getChanges()` | See npm: @approval/client |
| **cURL** | Command-line | curl | `curl -H "Authorization: Bearer $TOKEN" https://api.example.com/api/v1/changes` | Standard REST operations |

#### Change Management Endpoints

| Endpoint | HTTP | Purpose | Impl. Component |
| `/api/v1/changes` | GET | List pending changes | AP1.1.ChangePreviewController |
| `/api/v1/changes/{id}` | GET | Get change detail | AP1.1.ChangePreviewController |
| `/api/v1/changes/{id}/diff` | GET | Get diff visualization | AP1.1.ChangePreviewController |
| `/api/v1/changes/{id}/impact` | GET | Get impact analysis | AP1.1.ChangePreviewController |

#### Approval Endpoints

| Endpoint | HTTP | Purpose | Impl. Component |
|----------|------|---------|-----------------|
| `/api/v1/changes/{id}/approve` | POST | Approve change | AP2.1.ApprovalController |
| `/api/v1/changes/{id}/reject` | POST | Reject change | AP2.1.ApprovalController |
| `/api/v1/changes/{id}/approval-status` | GET | Get approval status | AP2.1.ApprovalController |

#### Application Endpoints

| Endpoint | HTTP | Purpose | Impl. Component |
|----------|------|---------|-----------------|
| `/api/v1/changes/{id}/apply` | POST | Apply changes | AP3.1.ApplicationController |
| `/api/v1/changes/{id}/verify` | GET | Verify application | AP3.1.ApplicationController |
| `/api/v1/changes/{id}/rollback` | POST | Rollback change | AP3.1.ApplicationController |
...
|-------|-----------|-------------|------|
| `preview.change.created` | ChangePreviewService | (Optional: NotificationService) | `PreviewChangeCreatedEvent` |
| `approval.decision.made` | ApprovalService | (Optional: NotificationService) | `ApprovalDecisionEvent` |
| `changes.applied` | ChangeApplicationService | (Optional: NotificationService) | `ChangesAppliedEvent` |
...
**Total Annual Growth:** ~475MB + indexes (~150MB) = ~625MB/year
...
|-----|---------|----------|------------|
| User Identifier | approval_decisions | 2-3 years | Encryption at rest, Role-Based Access Control |
...
*Authors: Swechcha Ambati (Software Engineer)*
